...
#or use
#pca to filter by basil and coriander see how data looks
grppb <- read.csv("Flavour_NEW2_mixedRT_PPB_low_in_dw+Ljp_rename_rem_ba_cors.csv")
#filter
library(dplyr)

#all compounds from norm rt as peak area. all species compounds still inc?
#this is non ppb data, how has ppb been worked out?
#done on excel? _ppb document available

gr <- filter(grppb, Species != "Basil") %>% droplevels()
gr <- filter(grppb, Species != "Coriander") %>% droplevels()
gr <- filter(grppb, Species != "Dandelion") %>% droplevels()
gr <- filter(grppb, Species != "Spinach") %>% droplevels()
...
#when remove basil and coriander
ions_acc <- ions_all[ ,c(1:59)] # selecting columns from csv
ions_acc.pca <- PCA(ions_acc, quali.sup=58) 
...
#try to do a biplot for 92 comps, cos >0.7
pdf('Flavour_biplot_DW+herbs_92comp_cos2_0.7.pdf')
fviz_pca_var(ions_acc.pca, axes = c(1, 2), geom = c("arrow", "text"),
             label = "all", invisible = "none", labelsize = 4,
             col.var = "black", alpha.var = 1, col.quanti.sup = "blue",
             col.circle = "grey70",
             select.var = list(name =NULL, cos2 = 0.7, contrib = NULL),
             jitter = list(what = "label", width = NULL, height = NULL))
dev.off()
#dev.new()

#pdf('Flavour_PCA+biplot_DW+herbs_92comp_cos2_0.7.pdf')
ggsave("Flavour_PCA+biplot_DW+herbs_59comp_cos2_0.7.pdf", dpi = 300, width = 17, height = 15 , units = "cm")
fviz_pca_biplot(ions_acc.pca, axes = c(1, 2), geom = c("arrow", "text"),
             label = "var", invisible = "none", labelsize = 4,
             habillage=ions_species,
...
pdf('Flavour_biplot_DW+herbs_remba_cor_59comp_cos2_0.7.pdf')
fviz_pca_var(ions_acc.pca, axes = c(1, 2), geom = c("arrow", "text"),
             label = "all", invisible = "none", labelsize = 4,
             col.var = "black", alpha.var = 1, col.quanti.sup = "blue",
             col.circle = "grey70",
...
#just duckweed biplots
#pdf('Flavour_PCA+biplot_DW+herbs_92comp_cos2_0.7.pdf')
ggsave("Flavour_PCA+biplot_DWONLY_59comp_cos2_0.7.pdf", dpi = 300, width = 17, height = 15 , units = "cm")
fviz_pca_biplot(ions_acc.pca, axes = c(1, 2), geom = c("arrow", "text"),
                label = "var", invisible = "none", labelsize = 4,
                habillage=ions_species,
                addEllipses=TRUE, ellipse.level=0.95,
...
pdf('Flavour_biplot_DWONLY_59comp_cos2_0.7.pdf')
ggsave("Flavour_biplot_DWONLY_59comp_cos2_0.7.pdf", dpi = 300, width = 17, height = 15 , units = "cm")
fviz_pca_var(ions_acc.pca, axes = c(1, 2), geom = c("arrow", "text"),
             label = "all", invisible = "none", labelsize = 4,
             col.var = "black", alpha.var = 1, col.quanti.sup = "blue",
             col.circle = "grey70",
...
#rem bas,cor
#rem bas, cor, spin, dan
ions_acc.pca<-prcomp(ions_acc[ ,1:59],center=T,scale=T) #
...
pca <- ggplot(ions_all, aes(x = PC1, y = PC2, color = ions_species)) +
  geom_point(aes(color=ions_species), size=5) +
  stat_ellipse() +
  theme_classic() +
...
ggsave("pca_flavour+herbs_col_newcol+Ljp.pdf", dpi = 300, width = 17, height = 15 , units = "cm") 
...
pcax <- ggplot(ions_all, aes(x = PC1, y = PC2, color = ions_species)) +
  ...
  geom_hline(yintercept=0, color="gray", linetype="dashed") + 
  geom_vline(xintercept=0, color="gray", linetype="dashed") +
  scale_x_continuous(limits = c(-25, 25), name="PC1 (31%)") +
  scale_y_continuous(limits = c(-25, 25), name="PC2 (22%)") +
  theme(legend.title=element_blank()) +
  theme(legend.text = element_text(face = "italic"))
  #autoplot(fit, label = TRUE, loadings.label = TRUE)
#ggsave("pca_flavour+herbs_col+Ljp_59comps.pdf", dpi = 300, width = 17, height = 15 , units = "cm") 
ggsave("pca_flavour+herbs_col+Ljp_59comps.tiff", dpi = 300, width = 17, height = 15 , units = "cm") 
pcax
dev.off()

library(ggbiplot)
fit <- princomp(ions_all, cor=TRUE)
biplot(fit)
...
pca2 <- ggplot(ions_all, aes(x = PC1, y = PC2, color = ions_species)) +
  ...
  theme(legend.text = element_text(face = "italic"))
pca2
ggsave("pca_flavour+dw_only_col_newcol+Ljp_59comp.pdf", dpi = 300, width = 17, height = 15 , units = "cm") 
#ggsave("pca_flavour+dw_only_col_newcol+Ljp_59comp.tiff", dpi = 300, width = 17, height = 15 , units = "cm") 
...
#dec2 nal missing so read in seperate to get species diffs
grppb <- read.csv("Flavour2_PPBlJP_FORHEAT_NOTINCBEFORE.csv")
library(dplyr)

#do group means to see which not in duckweeds
#mean for each specis type, if >1ppb removed
#one by one
#test
library(plyr)
...
#what been cut out here?
sum #92
grppb #see what diff cols are?
...
write.csv(resultac, "Flav_access_maincomp31_Ljp_sig.csv")
...
#for this dataset for heat map sig comps diff between dws
#need to remove herbs, avg by accessions
grppb <- read.csv("Flavour_NEW2_mixedRT_PPB_species_justsig+Ljp-forheatmap.csv")
...
write.csv(resultaspp, "Flav_species_maincomp26_Ljp_sig_avgs_forhetamap.csv")
...
write.csv(resultacc, "Flav_access_maincomp31_Ljp_sig_avgs_forhetamap.csv")

#read back in avged data removed herbs
grppb <- read.csv("Flav_access_maincomp31_Ljp_sig_avgs_forhetamap.csv")

#use reduced compounds, all <1ppb rem, basil coriander compounds higher than dw rem
...
ions_all <- resultacc[6:26]
ions_all <- resultaspp[6:26]
...
rownames(ions_sc) <- dw$Species

#assign row names from df
rownames(ions_sc) <- grppb2$Accession
rownames(ions_sc) <- grppb2$Species

#for accessions as heat map core 14 sig comps
rownames(ions_sc) <- grppb$Accession

#current for Ljp sig compounds access
rownames(ions_sc) <- resultacc$Accession

#current for Ljp sig compounds species
rownames(ions_sc) <- resultaspp$Species

#current for Ljp sig compounds sp
rownames(ions_sc) <- result_sig$Species

heatmap(ions_sc)
row <- heatmap(ions_sc, Colv = NA, Rowv = NA, scale="row")
#change color scheme yellow blue
colfunc <- colorRampPalette(c("blue", "yellow"))
heatmap(ions_sc,col=colfunc(11),scale="row")

col <- heatmap(ions_sc, Colv = NA, Rowv = NA, scale="col")
#change color scheme yellow blue
colfunc <- colorRampPalette(c("blue", "yellow"))
heatmap(ions_sc,col=colfunc(11),scale="col")
...
tiff('Heatmap_flavsig14_accessions_Ljp_avgs.tiff', units="in", width=20, height=12, res=300, compression = 'lzw')
pdf('Heatmap_flavsig26_species_Ljp_avgs.pdf')
pdf('Heatmap_flavsig26_accessions_Ljp_avgs.pdf')
par(mfrow = c(3, 3),  mar=c(5,4.5,4,2))
colfunc <- colorRampPalette(c("blue", "yellow"))
heatmap(ions_sc,col=colfunc(15),scale="row", Colv = NA, cexCol=1.2,margins=c(12,8))
